...
def limiarizacao(entrada, limiar): 
    imagem = np.zeros((len(entrada),len(entrada[0])), dtype=np.uint8)
    for i in range(0,len(entrada)):
        for j in range(0,len(entrada[0])):
            if entrada[i,j] >= limiar:
                imagem[i,j] = 255
    return imagem   

def limiarizacao_janela(entrada, min_l, max_l): 
    imagem = np.zeros((len(entrada),len(entrada[0])), dtype=np.uint8)
    for i in range(0,len(entrada)):
        for j in range(0,len(entrada[0])):
            if entrada[i,j] > min_l and entrada[i,j] < max_l:
                imagem[i,j] = 255
    return imagem   